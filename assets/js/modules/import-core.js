(function (global) {
  ...
  const REQUIRED_FIELDS = [
    ...
    ["optimizedMemoryTip", "memory_tip_记忆小窍门"],
    ["optimizedSpellingTip", "spelling_secret_拼写小秘密"],
  ...
  const TEMPLATE_HEADERS = [
    ...
    "memory_tip_记忆小窍门", "spelling_secret_拼写小秘密", "主动回忆题型", "展示模式",
  ...
  function validateRows(rows) {
    ...
    for (const item of rows) {
      ...
      if (booleanValue(item.showChunkTip, true) && !String(item.chunkTip || "").trim()) errors.push(`第${row}行缺少「chunk_tip_拆开记」；不需要时请将 show_chunk_tip 设为 FALSE`);
  ...
  global.ImportCore = Object.freeze({
    HEADER_ALIASES, REQUIRED_FIELDS, TEMPLATE_HEADERS, DISPLAY_MODES, SPELLING_MODES,
